...
export class XvelteApp {
    ...
    private pageHandlerMap = new Map<string, PageHandler<any>>();
    private pagePatternHandlerMap = new Map<pathToRegexp.MatchFunction<pathToRegexp.ParamData> | RegExp, PageHandler<any>>();

    constructor(template: string) {
        this.template = template;
    }

    page<Route extends string | RegExp>(route: Route, handler: PageHandler<Route>) {
    ...
    private async sendPage(req: IncomingMessage & {url: string}, res: ServerResponse): Promise<boolean> {
        let handler: PageHandler<any> | null = this.pageHandlerMap.get(req.url) ?? null;
...
type PageHandler<Route extends string | RegExp> = (event: RequestEvent<Route>) => MaybePromise<PageHandleData | null>;
type PageHandleData = {
    layouts?: {
        component: Component,
