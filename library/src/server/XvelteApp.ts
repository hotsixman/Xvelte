...
export class XvelteApp {
    ...
    page<Route extends string | RegExp, Props extends Record<string, any>, LayoutProps extends Record<string, any>[]>(route: Route, handler: PageHandler<Route, Props, LayoutProps>) {
        if (typeof (route) === "string") {
            let route_ = new URL(route, 'http://void').pathname;
    ...
    private getHandler(path: string) {
        ...
        if (handler) return { handler, params, isPageHandler: true } as const;
        ...
        if (handler) return { handler, params, isPageHandler: false } as const;
        ...
        return { handler, params, isPageHandler: false } as const;
    }
